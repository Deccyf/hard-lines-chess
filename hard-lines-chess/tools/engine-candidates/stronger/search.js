...
/**
 * Has the side to move anything but pawns and a king?
 *
 * Null-move pruning is unsound exactly where zugzwang lives, and zugzwang
 * lives in pawn endings. This is the cheap, standard test for "not one of
 * those": if there is a piece on the board for the side to move, being made
 * to move is not, on its own, a catastrophe.
 */
function hasNonPawnMaterial(board) {
  for (let sq = 0; sq < 128; sq++) {
    if (sq & 0x88) continue;
    const piece = board.squares[sq];
    if (!piece || colourOf(piece) !== board.turn) continue;
...
export class Engine {
  ...
  negamax(board, depth, alpha, beta, ply, line) {
    ...
    // ── NULL-MOVE PRUNING ────────────────────────────────────────────────
    //
    // Pass the turn and search what is left, shallower. If the opponent still
    // cannot get the score below beta after being handed a free move, then no
    // real move of ours is going to be worse than that either, and the whole
    // subtree can go. It is the largest single saving available to a search
    // this shape.
    //
    // AND IT IS WRONG IN ZUGZWANG, which is the reason for the material test.
    // In a king-and-pawn ending the obligation to move is the whole game: a
    // side that could pass would be fine and the side that must move loses. So
    // it is switched off wherever the side to move has nothing but pawns, and
    // in check, and at the root, where a pruned score is not a move.
    if (!inCheck && ply > 0 && depth >= 3 && beta < MATE_THRESHOLD && hasNonPawnMaterial(board)) {
      const reduction = 2 + (depth > 6 ? 1 : 0);
      board.makeNull();
      const score = -this.negamax(board, depth - 1 - reduction, -beta, -beta + 1, ply + 1, []);
      board.unmakeNull();
      if (this.stop) return 0;
    ...
    for (const move of ordered) {
      ...
      if (legal > 3 && depth >= 3 && !(moveFlags(move) & FLAG_CAPTURE) && !movePromo(move) && !inCheck) {
        ...
        // PRINCIPAL VARIATION SEARCH. The first move of a well-ordered list is
        // usually the best one, so it gets the full window; every move after it
        // is asked a cheaper question — "is it better than the best so far?" —
        // with a window one point wide, and only re-searched properly on the
        // rare occasion the answer is yes. The ordering already in this file is
        // what makes that a saving rather than a doubling.
        score = -this.negamax(board, depth - 1, -beta, -alpha, ply + 1, childLine);
      } else {
  ...
  quiesce(board, alpha, beta, ply) {
    ...
    // ── IN CHECK, THERE IS NOTHING TO STAND ON ───────────────────────────
    //
    // Standing pat means "I could stop here and be worth this much", and in
    // check that is false: you are not allowed to stop, and if nothing gets
    // you out you are checkmated. The old code stood pat anyway and looked
    // only at captures, so a position that was mate at the horizon came back
    // scored as ordinary material — the engine could not see it was being
    // mated one ply past where it stopped looking. In check, every legal move
    // is searched and there is no stand-pat.
    const inCheck = board.inCheck();
    let best = -MATE;
    ...
    const moves = this.order(board, board.generate(!inCheck), ply, 0);
    ...
    // Only reachable in check, where every legal move was generated: no legal
    // move and in check is mate, and mate is what has to be returned rather
    // than a material count of a position that does not continue.
