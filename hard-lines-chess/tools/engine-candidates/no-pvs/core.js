...
export class Board {
  ...
  /**
   * PASS THE TURN. Not a chess move — there is no such thing — but the search
   * needs one: if you could do nothing and STILL be winning by more than the
   * opponent could hope for, the position is so good that the rest of it does
   * not need looking at. That is null-move pruning, and this is the make/unmake
   * pair it needs.
   *
   * The en passant square goes, because it was created by the move before and
   * a pass does not keep it available. Nothing is pushed onto the repetition
   * list: a position reached by passing is not a position the game was ever in.
   */
  makeNull() {
    this.nulls.push({ ep: this.ep, hash: this.hash, hash2: this.hash2, halfmove: this.halfmove, fullmove: this.fullmove });
    if (this.ep >= 0) {
      this.hash = (this.hash ^ ZOBRIST_EP[this.ep]) >>> 0;
      this.hash2 = (this.hash2 ^ ZOBRIST2_EP[this.ep]) >>> 0;
    }
    this.ep = -1;
    this.turn = this.turn === WHITE ? BLACK : WHITE;
    this.hash = (this.hash ^ ZOBRIST_SIDE) >>> 0;
    this.hash2 = (this.hash2 ^ ZOBRIST2_SIDE) >>> 0;
    if (this.turn === WHITE) this.fullmove++;
    this.halfmove++;
  }

  unmakeNull() {
    const state = this.nulls.pop();
    if (!state) return;
    this.turn = this.turn === WHITE ? BLACK : WHITE;
    this.ep = state.ep;
